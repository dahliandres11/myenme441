...
def fire(laser_pin,turret_state):
    """
    Turns the laser on.
    """
...
def home_axis(motor, switch_pin, toward, away, fast_step=1, slow_step=2):# needs stepper class to be updated step function
    # Fast approach
    steps = 0
...
def handle_client(conn):
    ...
    # 3) POST /  -> UI actions from JS (laser, pan, tilt, auto_start, etc.)
        ...
        if action == 'laser':
            threading.Thread(
                target=fire,
                args=(laser_pin,turret_state),
                daemon=True
            ).start()
            print(f"Laser turned on for 3 seconds")
        if action == 'pan':
